...
function buildJiraSingleValue(
	...
	const t =
		budget <= 0
			...
			: title.length <= budget
				? title
				: title.slice(0, budget - 3) + "...";
	const value = JSON.stringify({ ...base, t });
	return value.length <= MAX ? value : baseJson;
}

function buildBulkJiraValue(
	base: Omit<JiraBulkPayload, "f">,
	...
	for (const f of findings) {
		...
		if (candidate.length <= MAX) {
			...
			if (budget > 3) {
				...
				if (
					JSON.stringify({ ...base, f: [...result, truncated] }).length <= MAX
				) {
...
function buildFixValue(id: string | null, context: ResultContext): string {
	...
	// Slack overflow option value limit: 150 chars
	const sha = (context.headSha ?? "").slice(0, 7);
	const ref = (context.headRef ?? context.branch ?? "").slice(0, 30);
	const base = (context.baseRef ?? "").slice(0, 20);
	// If still over 150 (unlikely with truncation), fall back to minimal payload
...
export function formatAuditResultBlocks(
	...
	if (summary.status === "failure") {
		return [
			{
				type: "header",
				text: { type: "plain_text", text: `Audit Failed — ${label}` },
			},
			{
				type: "context",
				elements: [
					{
						type: "mrkdwn",
						text: `Branch: \`${context.branch ?? "default"}\``,
					},
				],
			},
			{
				...
				text: {
					type: "mrkdwn",
					text: `*Error:* ${summary.error ?? "Unknown error"}`,
				},
			},
			{
				type: "actions",
				elements: [
					{
						type: "button",
						text: { type: "plain_text", text: "Retry Audit" },
						action_id: "a11y_retry_audit",
					},
				],
			},
		];
	...
	const total =
		summary.totalFindings + (summary.patternFindings?.totalFindings ?? 0);

	if (total === 0) {
		blocks.push(
			{
				type: "header",
				text: { type: "plain_text", text: `Audit Complete — ${label}` },
			},
			{
				type: "context",
				elements: [
					{
						type: "mrkdwn",
						text: `Branch: \`${context.branch ?? "default"}\` · 0 findings`,
					},
				],
			},
			{
				type: "section",
				text: { type: "mrkdwn", text: "No accessibility issues found." },
			},
		);
	} else {
		blocks.push(
			{
				type: "header",
				text: { type: "plain_text", text: `A11y Audit Results — ${label}` },
			},
			{
				type: "context",
				elements: [
					{
						type: "mrkdwn",
						text: `Branch: \`${context.branch ?? "default"}\` · ${total} findings`,
					},
				],
			},
		);
		...
		blocks.push({
			...
			elements: [
				{
					type: "mrkdwn",
					text: `:red_circle: Critical: ${combinedTotals.Critical}  :large_orange_circle: Serious: ${combinedTotals.Serious}  :large_yellow_circle: Moderate: ${combinedTotals.Moderate}  :large_blue_circle: Minor: ${combinedTotals.Minor}`,
				},
			],
		});

		if (
			summary.patternFindings &&
			summary.patternFindings.findings.length > 0
		) {
			blocks.push({ type: "divider" });
			blocks.push({
				type: "section",
				text: { type: "mrkdwn", text: "*Source Pattern Analysis*" },
			});
		...
		if (summary.findings && summary.findings.length > 0) {
			...
			blocks.push({
				type: "section",
				text: { type: "mrkdwn", text: "*DOM Audit*" },
			});
			const maxDom =
				20 - Math.min(summary.patternFindings?.findings.length ?? 0, 10);
			...
			domShown.forEach((f) => {
				...
				if (f.id) {
					...
					const blockId = (
						blockBase +
						(rfBudget > 0 ? (f.recommendedFix ?? "").slice(0, rfBudget) : "")
					).slice(0, 255);
					blocks.push({
						type: "section",
						block_id: blockId,
						...
						accessory: {
							...
							options: [
								{
									text: { type: "plain_text", text: "Fix with AI" },
									value: findingFixValue,
								},
								...(context.githubIssuesEnabled
									? [
											{
												text: {
													type: "plain_text",
	...
	if (total > 0) {
		...
		actions.push({
			type: "button",
			text: { type: "plain_text", text: "Fix All with AI" },
			action_id: "a11y_fix_all",
			value: fixContext,
			style: "primary",
		});
		if (context.githubIssuesEnabled)
			actions.push({
				type: "button",
				text: { type: "plain_text", text: "Create GitHub Issue" },
				action_id: "a11y_create_gh_issue",
				url: ghBulkUrl,
			});
...
function appendPatternFindings(
	...
	shown.forEach((f) => {
		...
		if (f.id) {
			...
			const patBlockId = `a11y_p_${f.id}|${f.file}|${f.line ?? ""}`.slice(
				0,
				255,
			);
			blocks.push({
	...
	if (patternFindings.totalFindings > shown.length) {
		blocks.push({
			...
			elements: [
				{
					type: "mrkdwn",
					text: `Showing ${shown.length} of ${patternFindings.totalFindings} pattern findings.`,
				},
			],
		});
	}
...
export function formatScanningBlocks(
	owner: string,
	repo: string,
	mode: string,
	branch?: string,
	...
	return [
		{
			type: "header",
			text: { type: "plain_text", text: `Auditing ${owner}/${repo}` },
		},
		{
			type: "context",
			elements: [
				{
					type: "mrkdwn",
					text: `Branch: \`${branch ?? "default"}\` · Mode: ${mode}`,
				},
			],
		},
		{
			type: "section",
			text: { type: "mrkdwn", text: "Scanning in progress…" },
		},
	];
}

export function formatFixProgressBlocks(
	owner: string,
	repo: string,
	findingIds: string,
): Record<string, unknown>[] {
	return [
		{
			type: "header",
			text: { type: "plain_text", text: `Applying Fix — ${owner}/${repo}` },
		},
		{
			type: "context",
			elements: [{ type: "mrkdwn", text: `Finding IDs: ${findingIds}` }],
		},
