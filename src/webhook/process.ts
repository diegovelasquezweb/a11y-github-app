...
async function handlePullRequestEvent(payload: {
	...
	if (
		...
		!headBranch.startsWith("a11y-fix/")
	) {
...
function buildWelcomeComment(context: "pr" | "issue" = "pr"): string {
	...
	return [
		...
		...commands,
		"",
		"> 📟 Runs [axe-core](https://www.deque.com/axe/) + [CDP](https://chromedevtools.github.io/devtools-protocol/) + [pa11y](https://pa11y.org/) against the live DOM, plus static [source pattern](https://vercel.com/design/guidelines) analysis.",
		"",
		"### Suggested workflow",
		"",
...
async function handleIssueEvent(payload: {
	...
	if (payload.issue?.pull_request) {
		return {
			status: 200,
			body: { ok: true, ignored: "issue event on pull request" },
		};
	}
	...
	if (!installationId || !owner || !repo || !issueNumber) {
		return {
			status: 400,
			body: { ok: false, error: "Missing required issue fields" },
		};
	}
...
function buildInitialAuditComment(
	requestedBy?: string,
	mode: AuditMode = "unified",
): string {
...
async function handleIssueCommentEvent(payload: {
	...
	if (!ALLOWED_AUDIT_ASSOCIATIONS.has(association)) {
		return {
			...
			body: {
				ok: true,
				ignored: `author association not allowed: ${association}`,
			},
		};
	}
	...
	if (fixCommand.requested) {
		...
		if (isPr) {
			...
			const resolved = await resolveBranchRef(
				octokit,
				owner,
				repo,
				fixCommand.branch,
	...
	if (!callbackUrl || !CONFIG.domAuditCallbackToken) {
		return {
			...
			body: {
				ok: false,
				error: "DOM audit callback configuration is incomplete",
			},
		};
	}
	...
	const initialBody = buildInitialAuditComment(
	...
	try {
		...
		if (command.auditMode === "source") {
			...
			await dispatchDomAuditWorkflow({
				...
				commentId,
				sourceScanEnabled:
					command.auditMode === "dom" ? false : CONFIG.sourcePatternsEnabled,
				...(auditBranch ? { branch: auditBranch } : {}),
			});
		}
	...
	try {
		...
		if (input.event === "issues") {
			return await handleIssueEvent(
				payload as Parameters<typeof handleIssueEvent>[0],
			);
		}

		if (input.event === "issue_comment") {
			return await handleIssueCommentEvent(
				payload as Parameters<typeof handleIssueCommentEvent>[0],
			);
		}
